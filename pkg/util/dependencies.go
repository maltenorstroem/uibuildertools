...
func GetDependencyList() []string {
	...
	for _, d := range viper.GetStringSlice("dependencies") {
		...
		if dep.Kind == UNKNOWN {
			fmt.Println(ScanForStringPosition(d, "./.uibuildertools"), "Config Error")
		...
		depconf.SetConfigName(".uibuildertools")
		...
		if err == nil {
			...
			// no .uibuildertools config in target dir, use the complete path
